...
const MyStats = () => {
	...
	// const { user } = useAuth0();
	const lineGraph = (type) => {
		const sortedRuns = myRuns.sort(
			(a, b) => new Date(a.date.split(', ').join('-')) - new Date(b.date.split(', ').join('-'))
		);
		const graphData = {
			id: 'runs',
			color: '#FF6600',
		...
		<React.Fragment>
			<Box className={classes.nav}>
				<ul>
					{myRuns.map((run) => {
						return (
							<li key={run.id} className={classes.list} onClick={handleRunDetails}>
								<RunDetails run={run} />
							</li>
						);
					})}
			...
			<Box display="flex" justifyContent="center">
				<Box className={classes.calendarContainer}>
					<Box className={classes.graphNav} display="flex" justifyContent="space-around">
						<Box className={showCal ? classes.base : classes.selected} onClick={handleShowCalGraph}>
							<span>
								Calendar <i className="fas fa-calendar-alt" />
							</span>
						</Box>
						<Box className={showDistance ? classes.base : classes.selected} onClick={handleShowDistGraph}>
							<span>
								Distance <i className="fas fa-running" />
							</span>
						</Box>
						<Box className={showCalories ? classes.base : classes.selected} onClick={handleShowCalorGraph}>
							<span>
				...
				<Button color="secondary" onClick={handleRunForm}>
					Add Run
				</Button>
			</Box>
		</React.Fragment>
